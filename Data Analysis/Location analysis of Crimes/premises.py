...
if __name__ == '__main__':
    ...
    # for all boroughs
    overall_a = lines.map(lambda x: (x[1], x[2], x[5], x[7], x[9], x[10], x[11], x[12], x[13], x[16]))
    overall = overall_a.map(lambda x: (x[3], x[16]))
    ...
    final_brooklyn.saveAsTextFile('brooklyn_Premises.csv')
    ...
    manhattan = manhattan_b.map(lambda x: (x[3], x[16]))
    ...
    final_staten.saveAsTextFile('staten_Premises.csv')
