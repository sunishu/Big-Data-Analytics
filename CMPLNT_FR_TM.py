...
def validity(x):
    ...
    try:
        ...
        if time.strptime(x,"%H:%M:%S"):
            ...
            if mat:
                return y,"TIME","START_OF_EVENT","VALID"
            else:
                raise ValueError
    except ValueError:
        return y,"TIME","START_OF_EVENT","INVALID"
...
if __name__ == "__main__":
    ...
    catg = lines.map(lambda x: '%s\t%s\t%s\t%s'%(validity(x[2])))
    count = lines.map(lambda x: ('%s\t'%validity(x[2])[3],1)).reduceByKey(lambda x,y: x+y)
